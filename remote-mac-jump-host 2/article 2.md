...
换个角度想。手机找不到家里 Mac 的门，这没法改。但有一样东西，手机能轻松找到，家里的 Mac 也能轻松找到，那就是一台有公网 IP 的云服务器。
...
下面的命令都在云服务器上跑。占位符按你自己的改：`<MAC_IP>` 是 Mac 的 Tailscale IP，`<MAC_USER>` 是你 Mac 的登录用户名。
...
> 一个提醒：tmux 只在最终干活的 Mac 上开一层就够。别在云服务器上也开 tmux 再套一层，前缀键会打架（Ctrl-b 要按两下）、状态栏会叠两条，纯添乱。
...
这里藏着一个非踩不可的坑：上面别名里 tmux 全写了全路径 `/opt/homebrew/bin/tmux`。原因是 `ssh 主机 -t "命令"` 跑的是一个不加载 `.zshrc` 的精简 shell，它的 PATH 里没有 Homebrew 那个目录，直接写裸 `tmux` 会报 `command not found`。写全路径就稳了（这是 Apple 芯片 Mac 的路径；Intel Mac 把它换成 `/usr/local/bin/tmux`）。
...
最省事的是让机器自己定时清。下面这个脚本会保留你当前正在用的那个连接，只杀掉闲置超过设定天数的孤儿（Mac 和 Linux 通用）：

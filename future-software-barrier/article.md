...
![](image/blackbox-layers.png)

<p align="center"><em>从口述到成文，中间隔着三层工程活。真正难抄的是这一整条链路</em></p>
...
靠极致的场景控制力建立护城河，这在软件行业不是新鲜事。

最典型的是 Raycast。

macOS 上苹果自带的 Spotlight 已经足够强大，可为什么还有一大批极客和开发者愿意付费用 Raycast？因为 Raycast 把“快捷启动”这条窄赛道做到了令人发指的程度。它不光找文件，还靠插件生态把开发者的日常工作流全塞进一个输入框。苹果不会为了这群人重构整个 Spotlight，Raycast 就在这个照不到的角落里成了无可替代的工具。

CleanShot X 也一样。

系统自带的截图工具足够好用，覆盖了绝大多数人的日常需求。CleanShot X 靠着把剩下 5% 的长尾需求做透，成了内容创作者的装机必备。长截图、贴图、滚动截屏、录屏转 GIF、标注工具，一个简单的截图动作被它做成了生产力工具。

还有更小的 PopClip。它只干一件事：你用鼠标选中一段文本，光标上方弹出一个小工具条，提供复制、翻译、格式转换。这个切入点窄到巨头根本不会多看一眼，但它生命力极强，因为交互路径太短了，短到用户一旦习惯就再也回不去。

![](image/narrow-track-map.png)

<p align="center"><em>系统自带的能力覆盖了 95%，剩下那 5% 里长出了一批不可替代的小工具</em></p>

这几个产品说的是同一件事。**在 AGI 时代，参数规模成不了护城河**，长久的壁垒来自对特定场景的绝对控制力，来自对交互体验没完没了的打磨。
